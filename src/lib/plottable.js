...
// with exactly one parameter (f(x):=..., or even f(t):=... - calling it back as "f(x)" always
// comes out as an expression in x regardless of what the definition itself calls its own
// parameter, since Giac substitutes whatever's actually passed), or a differential equation in
// y (checked with the exact same parser the plot panel's own diffeq row uses - see
// lib/plotDiffEq.js - so both agree on exactly which equations are offerable). Checked one
// line at a time so a multi-line initial-value problem (the ODE on one line, "y(0)=3" on the
// next - see wrapBareEquation in giac.js) still finds the ODE line; a line that isn't shaped
// like one (no derivative, or a 3rd-order+ one) just falls through to the next. Returns
// `{mode, expr}` ready for the plot panel's row shape, or null if nothing in this input is
// offerable this way.
export function plottableInputForEntry(entry) {
  ...
  if (def && def.kind === 'function' && def.params.length === 1) {
    return { mode: 'function', expr: `${def.name}(x)` };
  }
...
// that produced it. Returns `{mode, expr}`, or null if this entry's output isn't offerable at
...
export function plottableOutputForEntry(entry) {
  ...
  return isPlottableInX(raw) ? { mode: 'function', expr: raw } : null;
}
